  ...
  while ((arg = *argv++) && *arg++ == '-' && *arg) {
    while ((c = *arg++)) {
      ...
      case 'l':
        ...
        break;
        // set the maximum job size in bytes (default is 65535)
      case 'z':
        job_data_size_limit = parse_size_t(EARGF(flagusage("-z")));
        break;
        // set the size of each write-ahead log file (default is 10485760)
        /* (will be rounded up to a multiple of 512 bytes) */
      case 's':
        s->wal.filesize = parse_size_t(EARGF(flagusage("-s")));
        break;
        /* compact the binlog (default) */
      case 'c':
        s->wal.nocomp = 0;
        break;
        /* do not compact the binlog */
      case 'n':
        s->wal.nocomp = 1;
        break;
        /* fsync at most once every MS milliseconds (use -f0 for "always fsync") */
      case 'f':
